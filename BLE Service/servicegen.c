...
{
	  BLE_NAME_LENGTH + 2,  //User Define Scan response Data Length
    BLE_NAME_LENGTH + 1, 0x09, //长度，格式
		BLE_NAME_STRING
...
const uint8_t code SCAN_RESPONSE_DATA_INITIAL[32] =
{
    0x19,  //User Define Scan response Data Length

    0x18, 0x08, 0x41, 0x4D, 0x49, 0x43, 0x43, 0x4F,
    0x4D, 0x5F, 0x48, 0x49, 0x44, 0x5F, 0x44, 0x65,
    0x6D, 0x6F, 0x5F, 0x33, 0x32, 0x34, 0x5F, 0x4F,
...
//    0x44, 0x65, 0x6D, 0x6F, 0x5F, 0x33, 0x31, 0x36,
};
...
/* 属性 - Service (0xFB00)声明 自定义 温度服务 */
const uint8_t code ATT_HDL_USER_TEMPERTURE_SERVICE[] =
	
{
    0x00, _TEMPERTURE_HDL_OFFSET_BASE_,     // 属性句柄
    0x28, 0x00,                             // 属性UUID：0x2800（Service声明，由Bluetooth定义）
    0x02,                                   // 属性值长度
    0x00, 0xFB,                             // 属性值：Service UUID（Service UUID）
};


/* 属性 特性 1 Device Name - Characteristic (0xFB01)声明 Read & Write */
/* Device Name Characteristic */
const uint8_t code ATT_HDL_USER_01_DEVICENAME_RW01[] =
//const uint8_t code ATT_HDL_USER_DEVICENAME_RW01[] =	
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x01),         // 属性句柄
    0x28, 0x03,         // 属性UUID：0x2803（Characteristic声明，由Bluetooth定义）
    0x05,               // 属性值长度
    {                   // 属性值：1 byte 特性性能 + 2 byte 特性值属性句柄 + 2 byte 特性UUID
        //0x01 |    // Broadcast                                                  
        0x02 |    // Read                                                       
   ...
   (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x02 ), 0x00,         // （特性值）属性句柄
    0x01, 0xFB          // 特性UUID（ temperature Characteristic UUID0xFB02
};


/* 属性 - 特性值（Characteristic Value） */
const uint8_t code ATT_HDL_USER_DEVICENAME_RW01_INIT[] =
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x02 ),         // 属性句柄
    0xFB, 0x01,         // 属性UUID：0xFB01
    0x14,               // 属性值长度
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,      //属性值
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00,

};

uint8_t xdata att_HDL_USER_DEVICENAME_RW01[] =
{
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,      //属性值
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00,
...
/* 属性 - 特性描述符（Characteristic Value） */
const uint8_t code ATT_HDL_USER_DEVICENAME_CONFIGURATION_INIT[] = 
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x03 ),      //属性句柄                                                 
    0x29, 0x02,      //属性UUID 0x29 0x02 特性的声明 客户端特性的配置              
    0x02,            //属性的长度
    0x00,                                                                                       
    0x00                                                                                        
};

uint8_t xdata att_HDL_USER_DEVICENAME_CONFIGURATION[] =                                
{
    0x00,                                                                                              
    0x00                                                                                            
};
...
/* 属性 特性 2  - Characteristic (0xFB02)声明 Notify*/
/* temperature Characteristic */
const uint8_t code ATT_HDL_USER_01_TEMPERTURE_N01[] =
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x04),         // 属性句柄
    0x28, 0x03,         // 属性UUID：0x2803（Characteristic声明，由Bluetooth定义）
    0x05,               // 属性值长度
    {                   // 属性值：1 byte 特性性能 + 2 byte 特性值属性句柄 + 2 byte 特性UUID
        //0x01 |    // Broadcast                                                  
   ...
   (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x05 ), 0x00,         // （特性值）属性句柄
    0x02, 0xFB          // 特性UUID（ temperature Characteristic UUID0xFB02
};

/* 属性 - 特性值（Characteristic Value） */
const uint8_t code ATT_HDL_USER_TEMPERTURE_N01_INIT[] =
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x05 ),         // 属性句柄
    0xFB, 0x02,         // 属性UUID：0xFB02
    0x07,               // 属性值长度
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00     // 属性值

};

uint8_t xdata att_HDL_USER_TEMPERTURE_N01[] =
{
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00     // 属性值
};

/* 属性 - 特性描述符（Characteristic Value） */
const uint8_t code ATT_HDL_USER_TEMPERTURE_CONFIGURATION_INIT[] = 
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x06 ),      //属性句柄                                                 
    0x29, 0x02,      //属性UUID 0x29 0x02 特性的声明 客户端特性的配置              
    0x02,            //属性的长度
    0x00,                                                                                       
    0x00                                                                                        
...
uint8_t xdata att_HDL_USER_TEMPERTURE_CONFIGURATION[] =                                
{
    0x00,                                                                                              
    0x00                                                                                            
};
...
/* 属性 特性 3  - Characteristic (0xFB03)声明 Write*/
/* Command Characteristic */
const uint8_t code ATT_HDL_USER_01_COMMAND_W01[] =
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x07 ),         // 属性句柄
    0x28, 0x03,         // 属性UUID：0x2803（Characteristic声明，由Bluetooth定义）
    0x05,               // 属性值长度
    {                   // 属性值：1 byte 特性性能 + 2 byte 特性值属性句柄 + 2 byte 特性UUID
        //0x01 |    // Broadcast                                                  
   ...
   (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x08 ), 0x00,         // （特性值）属性句柄
    0x03, 0xFB          // 特性UUID（ temperature Characteristic UUID）
};

/* 属性 - 特性值（Characteristic Value） */
const uint8_t code ATT_HDL_USER_COMMAND_W01_INIT[] =
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x08 ),         // 属性句柄
    0xFB, 0x03,         // 属性UUID：0xFB03
    0x06,               // 属性值长度
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00    // 属性值

};

uint8_t xdata att_HDL_USER__COMMAND_W01[] =
{
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00    // 属性值
};
...
/* 属性 特性 4  - Characteristic (0xFB04)声明 Notify*/
/* Upload Record Characteristic */
const uint8_t code ATT_HDL_USER_01_UPLOAD_N01[] =
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x09 ),         // 属性句柄
    0x28, 0x03,         // 属性UUID：0x2803（Characteristic声明，由Bluetooth定义）
    0x05,               // 属性值长度
    {                   // 属性值：1 byte 特性性能 + 2 byte 特性值属性句柄 + 2 byte 特性UUID
        //0x01 |    // Broadcast                                                  
    ...
    (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x0A ), 0x00,         // （特性值）属性句柄
    0x04, 0xFB          // 特性UUID（ temperature Characteristic UUID）
};

/* 属性 - 特性值（Characteristic Value） */
const uint8_t code ATT_HDL_USER_UPLOAD_N01_INIT[] =
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x0A ),         // 属性句柄
    0xFB, 0x04,         // 属性UUID：0xFB04
    0x14,               // 属性值长度
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,      //属性值
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00,
...
uint8_t xdata att_HDL_USER_UPLOAD_N01[] =
{
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,      //属性值
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00,
...
/* 属性 - 特性描述符（Characteristic Value） */
const uint8_t code ATT_HDL_USER_UPLOAD_CONFIGURATION_INIT[] = 
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x0B ),      //属性句柄                                                 
    0x29, 0x02,      //属性UUID 0x29 0x02 特性的声明 客户端特性的配置              
    0x02,            //属性的长度
    0x00,                                                                                       
    0x00                                                                                        
...
uint8_t xdata att_HDL_USER_UPLOAD_CONFIGURATION[] =                                
{
    0x00,                                                                                              
    0x00                                                                                            
};

/* 属性 特性 5  - Characteristic (0xFB05)声明 Read & Notify*/
/* Status Characteristic */
const uint8_t code ATT_HDL_USER_01_STATUS_RN01[] =
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x0C ),         // 属性句柄
    0x28, 0x03,         // 属性UUID：0x2803（Characteristic声明，由Bluetooth定义）
    0x05,               // 属性值长度
    {                   // 属性值：1 byte 特性性能 + 2 byte 特性值属性句柄 + 2 byte 特性UUID
        //0x01 |    // Broadcast                                                  
    ...
    (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x0D ), 0x00,         // （特性值）属性句柄
    0x05, 0xFB          // 特性UUID（ Status Characteristic UUID）
};

/* 属性 - 特性值（Characteristic Value） */
const uint8_t code ATT_HDL_USER_STATUS_RN01_INIT[] =
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x0D ),         // 属性句柄
    0xFB, 0x05,         // 属性UUID：0xFB05
    0x06,               // 属性值长度
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00   // 属性值

};

uint8_t xdata att_HDL_USER_STATUS_RN01[] =
{
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00   // 属性值
};
...
/* 属性 - 特性描述符（Characteristic Value） */
const uint8_t code ATT_HDL_USER_STATUS_CONFIGURATION_INIT[] = 
{
    0x00, (_TEMPERTURE_HDL_OFFSET_BASE_ + 0x0E ),      //属性句柄                                                 
    0x29, 0x02,      //属性UUID 0x29 0x02 特性的声明 客户端特性的配置              
    0x02,            //属性的长度
    0x00,                                                                                       
    0x00                                                                                        
...
uint8_t xdata att_HDL_USER_STATUS_CONFIGURATION[] =                                
{
    0x00,                                                                                              
    0x00                                                                                            
};
...
const uint8_t code ATT_PERMISSION_HDL_USER_DEVICENAME_RW01_INIT[] =
{
    {
        0x01 |      // Read,  Notify
        0x02 |      // Write, Indicate
        //0x04 |      // Encryption Read,  Encryption Notify
        //0x08 |      // Encryption Write, Encryption Indicate
        //0x10 |      // Authentication Read,  Authentication Notify
        //0x20 |      // Authentication Write, Authentication Indicate
        //0x40 |      // Authorization Read
        //0x80 |      // Authorization Write
        0x00
    },
    {
        0x01 |        //16 UUID 
...
const uint8_t code ATT_PERMISSION_HDL_USER_DEVICENAME_CONFIGURATION_INIT[] =
{
    {
        0x01 |      // Read,  Notify
        0x02 |      // Write, Indicate
...
const uint8_t code ATT_PERMISSIONATT_HDL_USER_TEMPERTURE_N01_INIT[]=
{
    {
        0x01 |      // Read,  Notify
        0x02 |      // Write, Indicate
...
const uint8_t code ATT_PERMISSION_HDL_USER_TEMPERTURE_CONFIGURATION_INIT[]=
{
    {
        0x01 |      // Read,  Notify
        0x02 |      // Write, Indicate
...
const uint8_t code ATT_PERMISSION_HDL_USER_COMMAND_W01_INIT[]=
{
    {
        0x01 |      // Read,  Notify
        0x02 |      // Write, Indicate
...
const uint8_t code ATT_PERMISSION_HDL_USER_UPLOAD_N01_INIT[] =
{
    {
        0x01 |      // Read,  Notify
        0x02 |      // Write, Indicate
...
const uint8_t code ATT_PERMISSION_HDL_USER_UPLOAD_CONFIGURATION_INIT[] =
{
    {
        0x01 |      // Read,  Notify
        0x02 |      // Write, Indicate
...
const uint8_t code ATT_PERMISSION_HDL_USER_STATUS_RN01_INIT[] =
{
    {
        0x01 |      // Read,  Notify
        0x02 |      // Write, Indicate
...
const uint8_t code ATT_PERMISSION_HDL_USER_STATUS_CONFIGURATION_INIT[] =
{
    {
        0x01 |      // Read,  Notify
        0x02 |      // Write, Indicate
...
#endif    //_PROFILE_USER_TEMPERTURE_
...
////////////////////////temperature//////////////////////////////////
    ...
    ATT_HDL_USER_TEMPERTURE_SERVICE,
    ATT_HDL_USER_01_DEVICENAME_RW01,
    ATT_HDL_USER_DEVICENAME_RW01_INIT,
    ATT_HDL_USER_DEVICENAME_CONFIGURATION_INIT,
		ATT_HDL_USER_01_TEMPERTURE_N01,
		ATT_HDL_USER_TEMPERTURE_N01_INIT,
		ATT_HDL_USER_TEMPERTURE_CONFIGURATION_INIT,
		ATT_HDL_USER_01_COMMAND_W01,
		ATT_HDL_USER_COMMAND_W01_INIT,
		ATT_HDL_USER_01_UPLOAD_N01,
		ATT_HDL_USER_UPLOAD_N01_INIT,
		ATT_HDL_USER_UPLOAD_CONFIGURATION_INIT,
    ATT_HDL_USER_01_STATUS_RN01,
    ATT_HDL_USER_STATUS_RN01_INIT,
		ATT_HDL_USER_STATUS_CONFIGURATION_INIT,
...
////////////////////////temperature//////////////////////////////////
        ...
        &ATT_HDL_USER_TEMPERTURE_SERVICE[5],
        &ATT_HDL_USER_01_DEVICENAME_RW01[5],
				att_HDL_USER_DEVICENAME_RW01,
        att_HDL_USER_DEVICENAME_CONFIGURATION,
				 
			 &ATT_HDL_USER_01_TEMPERTURE_N01[5],
			 att_HDL_USER_TEMPERTURE_N01,
			 att_HDL_USER_TEMPERTURE_CONFIGURATION,
				 
			 &ATT_HDL_USER_01_COMMAND_W01[5],
			 att_HDL_USER__COMMAND_W01,
				 
			 &ATT_HDL_USER_01_UPLOAD_N01[5],
			 att_HDL_USER_UPLOAD_N01,
			 att_HDL_USER_UPLOAD_CONFIGURATION,
				 
			 &ATT_HDL_USER_01_STATUS_RN01[5],
			 att_HDL_USER_STATUS_RN01,
			 att_HDL_USER_STATUS_CONFIGURATION,
...
////////////////////////temperature//////////////////////////////////
    ...
    ATT_PERMISSION_CHARACTERISTIC,
		ATT_PERMISSION_HDL_USER_DEVICENAME_RW01_INIT,
		ATT_PERMISSION_HDL_USER_DEVICENAME_CONFIGURATION_INIT,
		
		ATT_PERMISSION_CHARACTERISTIC,
		ATT_PERMISSIONATT_HDL_USER_TEMPERTURE_N01_INIT,
		ATT_PERMISSION_HDL_USER_TEMPERTURE_CONFIGURATION_INIT,
		
		ATT_PERMISSION_CHARACTERISTIC,
		ATT_PERMISSION_HDL_USER_COMMAND_W01_INIT,
		
		ATT_PERMISSION_CHARACTERISTIC,
		ATT_PERMISSION_HDL_USER_UPLOAD_N01_INIT,
		ATT_PERMISSION_HDL_USER_UPLOAD_CONFIGURATION_INIT,
		
		ATT_PERMISSION_CHARACTERISTIC,
		ATT_PERMISSION_HDL_USER_STATUS_RN01_INIT,
		ATT_PERMISSION_HDL_USER_STATUS_CONFIGURATION_INIT,
